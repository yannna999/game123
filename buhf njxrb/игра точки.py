...
class RewardsBombs():
    ...
    def run(self):
        while True:
            ...
            # движение фиолетовых бомб
            for i in range(len(self.violet_positions)):
                self.violet_positions[i][1] += self.violet_speed
            ...
            if random.random() < 0.02:
                ...
                if num % 2 == 0:
                    self.violet_positions.append([x, 0, False])
                else:
                    self.violet_positions.append([x, 0, True])

            # проверка столкновений с игроком
            for pos in self.violet_positions:
                if pos[2]:
                    if abs(pos[0] - self.green_pos[0]) <= 20 and abs(pos[1] - self.green_pos[1]) <= 20:
                        self.score += 1
                        self.violet_positions.remove(pos)
                else:
                    if (pos[0] - self.green_pos[0]) ** 2 + (pos[1] - self.green_pos[1]) ** 2 < 400:
                        self.game_over()

            # убираем бомбы за пределами окна
            self.violet_positions = [pos for pos in self.violet_positions if pos[1] < self.screen_height]
            self.screen.fill((0, 0, 0))

            for pos in self.violet_positions:
                if pos[2]:
                    pygame.draw.polygon(self.screen, (100, 0, 155), [[pos[0], pos[1]-10], [pos[0]+10, pos[1]+10], [pos[0]-10, pos[1]+10]])
                else:
                    pygame.draw.circle(self.screen, (150, 9, 32), pos[:2], 10)

            pygame.draw.circle(self.screen, (47, 90, 255), self.green_pos, 10)
